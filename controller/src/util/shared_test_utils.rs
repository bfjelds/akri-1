    ...
    const LIST_PREFIX: &str = r#"
{
    ...
    pub fn configure_get_instances(mock: &mut MockKubeImpl, result_file: &'static str, listify_result: bool) {
        ...
        mock.expect_get_instances().times(1).returning(move || {
            ...
            let instance_list_json = if listify_result { listify_kube_object(&json) } else { json };
    ...
    pub fn configure_update_instance(
        ...
        mock.expect_update_instance()
            ...
            .returning(move |_, _, _| if result_error { Err(None.ok_or("failure")?) } else { Ok(()) });
    }
    ...
    pub fn configure_find_services(
        ...
        mock.expect_find_services().times(1).withf(move |selector| selector == svc_selector).returning(move |_| {
            if result_error {
                Err(None.ok_or("failure")?)
            } else {
    ...
    pub fn configure_add_service(
        ...
        mock.expect_create_service()
            .withf(move |svc_to_create, ns| {
                svc_to_create.metadata.as_ref().unwrap().name.as_ref().unwrap() == svc_name
                    && svc_to_create.metadata.as_ref().unwrap().labels.as_ref().unwrap().get(label_id).unwrap()
                        == label_value
                    && ns == namespace
            })
    ...
    pub fn configure_remove_service(mock: &mut MockKubeImpl, svc_name: &'static str, svc_namespace: &'static str) {
        trace!("mock.expect_remove_service svc_name:{}, svc_namespace={}", svc_name, svc_namespace);
        mock.expect_remove_service()
            .times(1)
            .withf(move |svc_to_remove, namespace| svc_to_remove == svc_name && namespace == svc_namespace)
            .returning(move |_, _| Ok(()));
    }
    ...
    pub fn configure_update_service(
        ...
        trace!("mock.expect_update_service name:{} namespace:{} error:{}", svc_name, svc_namespace, result_error,);
    ...
    pub fn configure_add_pod(
        ...
        mock.expect_create_pod()
            ...
            .withf(move |pod_to_create, namespace| {
                pod_to_create.metadata.as_ref().unwrap().name.as_ref().unwrap() == pod_name
                    && pod_to_create.metadata.as_ref().unwrap().labels.as_ref().unwrap().get(label_id).unwrap()
                        == label_value
    ...
    pub fn configure_remove_pod(mock: &mut MockKubeImpl, pod_name: &'static str, pod_namespace: &'static str) {
        trace!("mock.expect_remove_pod pod_name:{} pod_namespace:{}", pod_name, pod_namespace);
        mock.expect_remove_pod()
            .times(1)
            .withf(move |pod_to_remove, namespace| pod_to_remove == pod_name && namespace == pod_namespace)
            .returning(move |_, _| Ok(()));
    }
}
